...
/**
 * หักเครดิต — กันติดลบ
 * ถ้าจำนวนที่จะหักมากกว่าเครดิตจริง (raw) → throw error ไม่หักเลย
 * @param {string} email
 * @param {number} amount  ค่าบวก (จำนวนที่จะหัก)
 * @returns {Promise<number>} ยอดเครดิตใหม่หลังหัก
 */
export async function deductCredit(email, amount) {
  const amt = Math.max(0, Number(amount) || 0);
  if (amt === 0) return await getUserCreditRaw(email);
  const raw = await getUserCreditRaw(email);
  if (raw < amt) {
    const err = new Error('เครดิตไม่เพียงพอ');
    err.code = 'INSUFFICIENT_CREDIT';
    err.balance = Math.max(0, raw);
    err.needed = amt;
    err.shortBy = amt - Math.max(0, raw);
    throw err;
  }
  const r = await command('INCRBYFLOAT', KEY_CREDIT(email), -amt);
  return Number(r) || 0;
}

/* รหัสสมาชิกแบบ OP-XXXX — INCR ตัวนับต่อเนื่อง (เริ่ม 0001) */
export async function nextMemberCode() {
  const r = await command('INCR', 'members:counter');
  const n = typeof r === 'string' ? Number(r) : r;
  return `OP-${String(n).padStart(4, '0')}`;
}

/* เติมรหัสสมาชิกให้บัญชีเก่าที่ยังไม่มี — เรียงตามวันสมัคร (เก่าก่อน) */
export async function backfillMemberCodes() {
  const accounts = await listAccounts();
  // เรียงเก่าก่อน (สมัครก่อนได้เลขน้อยกว่า)
  accounts.sort((a, b) => String(a.createdAt || '').localeCompare(String(b.createdAt || '')));
  let assigned = 0, skipped = 0;
  for (const a of accounts) {
    if (a.memberCode) { skipped++; continue; }
    const key = a.internalId || a.idNumber;
    if (!key) { skipped++; continue; }
    const memberCode = await nextMemberCode();
    await saveAccount(key, { ...a, memberCode });
    assigned++;
  }
...
/* OCR cache — เก็บผลการอ่านบัตร ปชช ไว้ระหว่างที่ user กำลังกรอกฟอร์มสมัคร
   เพื่อไม่ต้องเรียก iApp ซ้ำตอน submit จริง (ประหยัดเครดิต) */
const KEY_OCR = (token) => `ocrCache:${String(token||'').replace(/[^A-Za-z0-9_-]/g,'')}`;

export async function saveOcrCache(token, data, ttlSec = 1800) {
  await command('SET', KEY_OCR(token), JSON.stringify(data), 'EX', String(ttlSec));
}
export async function getOcrCache(token) {
  const raw = await command('GET', KEY_OCR(token));
  if (!raw) return null;
  try { return JSON.parse(raw); } catch { return null; }
}
export async function delOcrCache(token) {
  await command('DEL', KEY_OCR(token));
}

...
const OTP_SECRET = () => (process.env.TBS_OTP_SECRET || '').trim();

async function callTbsOtp(fullUrl, params, timeoutMs = 15000) {
  if (!OTP_KEY() || !OTP_SECRET()) throw new Error('TBS_OTP_KEY / TBS_OTP_SECRET ยังไม่ได้ตั้งค่า');
  // TBS OTP API v2 ต้องการทั้ง key + secret ของ OTP application ใน body (ไม่ใช่ Basic Auth)
  const bodyWithAuth = { ...params, secret: OTP_SECRET() };
  const ctrl = new AbortController();
...
export async function verifyOtp(token, pin) {
  ...
  const key = OTP_KEY();
  if (!key) throw new Error('TBS_OTP_KEY ยังไม่ได้ตั้งค่า');
  // /otp/verify ก็ต้องการ key + secret เหมือน /otp/request
  const r = await callTbsOtp(`${TBS_OTP_BASE}/otp/verify`, { key, token, pin: String(pin) });
